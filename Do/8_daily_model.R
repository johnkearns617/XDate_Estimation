...
feb_forecast = bind_cols(
  ...
  bind_rows(data.frame(record_calendar_day=as.numeric(day(seq(as.Date("2025-02-12"),as.Date("2025-02-28"),by=1))))) %>% 
  left_join(outlay_daily_df %>% 
  ...
  mutate(avg_share_outlay=avg_share_outlay/avg_share_outlay[n()],
         ...
         outlay_mtd_amt=outlay_mtd_amt*tail(na.omit(outlay_extrap_total),1)*avg_share_outlay[record_calendar_day==11]/tail(na.omit(outlay_mtd_amt),1)*1000,
         outlay_day_amt=outlay_day_amt*tail(na.omit(outlay_extrap_total),1)*avg_share_outlay[record_calendar_day==11]/tail(na.omit(outlay_mtd_amt),1)*1000,
         ...
         receipt_day_amt=receipt_day_amt*tail(na.omit(receipts_extrap_total),1)*avg_share_receipt[record_calendar_day==11]/tail(na.omit(receipt_mtd_amt),1)*1000,
         outlay_day_amt=outlay_day_amt*tail(na.omit(receipts_extrap_total),1)*avg_share_receipt[record_calendar_day==11]/tail(na.omit(receipt_mtd_amt),1)*1000,
...
forecast_component = function(nowcast_object,nowcast_total_object,daily_df,cbo_monthly_proj_col,component_abbrev){
  ...
  y=reg_df %>% drop_na() %>% select(actual) %>% pull()
  arima1 = auto.arima(y,
                      xreg= as.matrix(reg_df %>% drop_na() %>% select(colnames(reg_df)[3:ncol(reg_df)])),
                      seasonal=TRUE,
                      biasadj=TRUE)
  
  arima1 %>% 
    forecast(xreg=
               as.matrix(reg_df %>% filter(date>max(nowcast_object[[3]]$date)&date<=max(nowcast_object[[3]]$date)%m+%years(2)) %>% select(colnames(reg_df)[3:ncol(reg_df)]))) %>% 
    autoplot()
...
forecast_list[["revenue_Corporate Income Taxes"]] = forecast_component(nowcast_corporate_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Corporate Income Taxes","corp")[[1]]
forecast_list[["revenue_Estate and Gift Taxes"]] =forecast_component(nowcast_estate_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Estate and Gift Taxes",'estate')[[1]]
forecast_list[["revenue_Excise Taxes"]]=forecast_component(nowcast_excise_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Excise Taxes","excise")[[1]]
forecast_list[["revenue_Customs Duties"]]=forecast_component(nowcast_customs_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Customs Duties","customs")[[1]]
forecast_list[["revenue_Individual Income Taxes"]]=forecast_component(nowcast_individual_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Individual Income Taxes","individ")[[1]]
forecast_list[["revenue_Miscellaneous Receipts"]]=forecast_component(nowcast_misc_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Miscellaneous Receipts","misc")[[1]]
forecast_list[["revenue_Payroll Taxes"]]=forecast_component(nowcast_payroll_receipts,nowcast_total_receipts,receipt_daily_df,"revenue_Payroll Taxes","payroll")[[1]]

forecast_list[["outlay_Medicaid"]] = forecast_component(nowcast_medicaid_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Medicaid","medicaid")[[1]]
forecast_list[["outlay_Medicare"]] =forecast_component(nowcast_medicare_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Medicare",'medicare')[[1]]
forecast_list[["outlay_Net Interest"]]=forecast_component(nowcast_interest_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Net Interest","interest")[[1]]
forecast_list[["outlay_Social Security"]]=forecast_component(nowcast_ss_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Social Security","ss")[[1]]
forecast_list[["outlay_Defense Discretionary"]]=forecast_component(nowcast_defense_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Defense Discretionary","defense")[[1]]
forecast_list[["outlay_Other"]]=forecast_component(nowcast_other_outlay,nowcast_total_outlays,outlay_daily_df,"outlay_Other","other")[[1]]
