...
#                 pivot_wider(names_from=series_id,values_from=value) %>%
#                 select(date,A261RX1Q020SBEA:SLCEC1)) %>%
#     left_join(gdpnow_data %>%
#                   filter(release_date<=dat&date<=dat) %>%
...
#   )) %>%
#   rowwise() %>%
#   mutate(diff=gdp_data[[var_name_lng]][gdp_data$prediction_date==prediction_date&gdp_data$date==date],
#          diff=diff-ifelse(
...
#            (gdp_data[['gdp']][gdp_data$prediction_date==prediction_date&gdp_data$date==date]/100+1)*fcast_df[['GDPC1']][fcast_df$date==(date %m-% months(6))]
#          ),
