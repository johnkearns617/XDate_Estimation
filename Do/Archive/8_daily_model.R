...
# Forecast the current fiscal year
forecast_list = list()

forecast_list[["revenue_Corporate Income Taxes"]] = nowcast_daily_budget_receipt(dts,receipts,end_date,"Corporate Income Taxes","nowcast_corporate_receipts")
forecast_list[["revenue_Estate and Gift Taxes"]] = nowcast_daily_budget_receipt(dts,receipts,end_date,"Estate and Gift Taxes","nowcast_estate_receipts")
forecast_list[["revenue_Excise Taxes"]]= nowcast_daily_budget_receipt(dts,receipts,end_date,"Excise Taxes","nowcast_excise_receipts")
forecast_list[["revenue_Customs Duties"]]= nowcast_daily_budget_receipt(dts,receipts,end_date,"Customs Duties","nowcast_customs_receipts")
forecast_list[["revenue_Individual Income Taxes"]]= nowcast_daily_budget_receipt(dts,receipts,end_date,"Individual Income Taxes","nowcast_individual_receipts")
# think about if I like how the algorithm deals with COVID stimulus checks in 2020, but not a major concern because it just needs to work well moving forward
# think about how I would have added a manual adjustment during COVID, if it were to happen again
forecast_list[["revenue_Miscellaneous Receipts"]]= nowcast_daily_budget_receipt(dts,receipts,end_date,"Miscellaneous Receipts","nowcast_misc_receipts")
forecast_list[["revenue_Payroll Taxes"]]= nowcast_daily_budget_receipt(dts,receipts,end_date,"Payroll Taxes","nowcast_payroll_receipts")

forecast_list[["outlay_Medicaid"]] = nowcast_daily_budget_outlay(dts,outlays,end_date,"Medicaid","nowcast_medicaid_outlay")
forecast_list[["outlay_Medicare"]] =nowcast_daily_budget_outlay(dts,outlays,end_date,"Medicare","nowcast_medicare_outlay")
forecast_list[["outlay_Net Interest"]]=nowcast_daily_budget_outlay(dts,outlays,end_date,"Net Interest","nowcast_interest_outlay")
forecast_list[["outlay_Social Security"]]=nowcast_daily_budget_outlay(dts,outlays,end_date,"Social Security","nowcast_ss_outlay")
forecast_list[["outlay_Defense Discretionary"]]=nowcast_daily_budget_outlay(dts,outlays,end_date,"National Defense","nowcast_defense_outlay")
forecast_list[["outlay_Other"]]=nowcast_daily_budget_outlay(dts,outlays,end_date,"Other Spending","nowcast_other_outlay")
